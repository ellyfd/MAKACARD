...
- **Mission Run**：以任務交付物、必要席位、跨單位依賴與可結算的事件牌推進五回合；可選擇「依職務自動建議」或手動打出戰術牌，並在回合結束時結算單位覆蓋與任務閉環。結束後可複製任務草案紀錄，供下一步 owner、期限與決策確認使用。牌面僅以 JD 與單位席位顯示證據、錯配與任務效果，不做人格合拍度評分。
...
- `org-membership-data.js`：僅將「既有部門名稱或完整部門路徑與正式節點完全相同」的成員正規化至節點，並保留來源與版本；不推測直屬關係。
- org-release-data.js：公開組織快照的發布版號與來源檔清單；來源版本不等同正式生效日。
...
- **Phase 1**：依 PDF／人令校正正式組織、職位與直屬／兼任關係；匯入格式見 [`docs/phase-1-source-intake.md`](docs/phase-1-source-intake.md)。
...
- **Phase 5+**：草案組織比較、登入權限與企業版資料治理，規格見 [`docs/phase-5-enterprise-readiness.md`](docs/phase-5-enterprise-readiness.md)。
